...
void AgendaUI::changeColor(textColor color) { printf("\033[3%dm", color); }

void AgendaUI::noneColor(void) { printf("\033[1m"); }
...
bool AgendaUI::userLogIn(void) {
  cout << "[log in] [username]" << endl << "[log in] ";
  string username = getOperation();
  cout << "[log in] [password]" << endl << "[log in] ";
...
void AgendaUI::createMeeting(void) {
  ...
  cout << "[create meeting] [title] [start time(yyyy-mm-dd/hh:mm)] [end "
...
void AgendaUI::quitMeeting(void) {
  cout << endl << "[quit meeting] [meeting title]" << endl << "[quit meeting] ";
  string title = getOperation();
  cout << "[quit meeting] ";
...
void AgendaUI::listAllSponsorMeetings(void) {
  ...
  cout << endl << "[list all sponsor meetings]";
  printMeetings(meetings, "|All Meetings|");
}
...
void AgendaUI::listAllParticipateMeetings(void) {
  ...
  cout << endl << "[list all participator meetings]";
  printMeetings(meetings, "|All Meetings|");
}
...
void AgendaUI::queryMeetingByTimeInterval(void) {
  cout << endl
       << "[query meeting] [start time(yyyy-mm-dd/hh:mm)] [end "
          "time(yyyy-mm-dd/hh:mm)]"
       << endl
       << "[query meeting] ";
  string startDate = getOperation();
...
void AgendaUI::deleteAllMeetings(void) {
  cout << endl << "[delete all meeting] [meeting title]" << endl;
...
void AgendaUI::printMeetings(const std::list<Meeting> &t_meetings,
  ...
  if (t_meetings.empty()) {
    ...
    printf("%-12s%-12s%-20s%-20s%-18s", "Title", "Sponsor", "Start time",
  ...
  for (auto meeting : t_meetings) {
    printf("%-12s%-12s%-20s%-20s", meeting.getTitle().c_str(),
